...
## 2026-08-27 · UI 공개 자료 정합성 보완

- USB 분리 후 무선 ADB 단독 경로에서 설치 APK를 읽어 공개 v1.4.1 SHA-256과 일치 확인
- 미푸시 UI 코드나 다른 설치 APK는 없음을 확인하고, 기능 변경 없이 문서·화면 자료 범위로 한정
- v1.4.0에 머문 디자인 QA를 현행 v1.4.1 기준으로 보완하고 이전 검증 이력은 보존
- 한·영 README·상세 설명서·Release 설명에 숫자/손잡이 후광, 연속 막대, 7개 프리셋, 버튼 활성/비활성 상태와 구현 버전을 명시
- 초안 `0` 선택과 실제 `+75` 적용을 구분하는 영어 화면 추가 및 공개 이미지 7개의 원본 픽셀 일치 검사 PASS
