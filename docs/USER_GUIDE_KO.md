...
| 위치·항목 | 의미와 예 |
|---|---|
| 상단 `적용 준비 완료` | Shizuku 연결·자동 밝기 조건이 준비됐다는 뜻입니다. 이 문구만으로 보정 관리 중이라고 판단하지 않습니다. |
| 가운데 큰 숫자 | 지금 선택한 초안입니다. `0`을 선택해도 적용 버튼을 누르기 전에는 기존 보정이 유지됩니다. |
| `현재 적용값 0.75` | 적용 중인 보정 강도 `+75`를 내부 단위로 표시합니다. 패널 밝기 75%라는 뜻이 아닙니다. |
| 밝은 `선택한 보정 적용` | 선택값이 현재 적용값과 달라 적용할 수 있습니다. 누르면 선택값이 실제 보정에 반영됩니다. |
| 낮은 강조의 `현재 +75 적용 중` | 선택값과 적용값이 같아 중복 적용을 막는 상태입니다. 그라데이션이 약해지는 것은 의도된 상태 구분입니다. |
| `보정 일시 중지`·`원래 값 복원 대기` | 저장된 숫자가 보이더라도 정상 관리 중으로 해석하지 않습니다. Shizuku 연결을 복구한 뒤 상태를 확인합니다. |

숫자·손잡이의 후광, 연속 트랙, 7단계 프리셋과 다층 적용 버튼은 v1.4.0부터 포함된 디자인입니다. v1.4.1은 이 디자인에 연결·복원 상태 표시를 보강했습니다. 화면 글꼴은 휴대폰의 시스템 글꼴에 따라 달라질 수 있습니다.
...
위 화면은 `0`을 **선택만** 했고 `현재 적용값 0.75`는 아직 유지 중입니다. `+75` 프리셋을 다시 선택하면 적용하지 않고도 초안을 현재 값으로 되돌릴 수 있습니다.
...
1. Shizuku를 엽니다.
2. 무선 디버깅 또는 5단계 USB ADB 절차로 Shizuku를 다시 시작합니다.
3. 자동 밝기 보정 앱을 엽니다.
...
- 관리 여부, 마지막 보정 강도, 재부팅 후 재적용 선택, 복원 대기와 알림 권한 요청 여부를 기기에 저장합니다. 전체 목록은 [개인정보 및 권한](PRIVACY.md)을 참고하십시오.
