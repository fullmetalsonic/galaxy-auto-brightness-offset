...
## DBG-014 현행 UI와 공개 설명의 버전·상태 설명 불일치

- 증상: v1.4.1이 공개됐지만 디자인 QA 제목은 v1.4.0에 머물고, README와 한·영 안내에 적용 전/중 버튼의 강조 차이가 충분히 설명되지 않음
- 재현 방법: 설치 APK·Release 해시를 대조한 뒤 README, 디자인 QA 제목, 한·영 선택 화면 설명을 비교
- 직접 원인: 기능 릴리스 체크리스트에 기존 시각 변경의 계승 여부와 선택값/적용값 설명 확인이 빠짐
- 확인 증거: 조사 시작 시 로컬·원격 main은 같은 커밋, 설치 APK와 공개 APK의 SHA-256도 일치. UI 소스 누락이 아니라 문서 보완 항목으로 확인
- 영향 범위: 공개 README, 한·영 상세 설명서, 디자인 QA와 Release 설명; 앱 기능·버전에는 변경 없음
- 잘못된 기존 접근: APK 게시만으로 기존 UI 변경과 현재 화면 상태 설명도 충분히 전달됐다고 판단. 이미지 미리보기만 보고 손상을 단정하지 않아야 함
- 수정 내용: 현행 UI 안내, 선택 초안 0/실제 적용 +75 구분, 영어 선택 화면, 이미지 원본·출력 해시와 자르기 명세 추가
- 자동 재발방지 장치: 공개 PNG 해시·크기와 앱/설명서 제목 버전을 확인하는 CI 단계, 비공개 원본과의 로컬 RGBA 전체 비교
- 재시험 결과: 7개 자르기 출력 모두 픽셀 차이 0, 공개 자료 검사 PASS, 1픽셀 변경을 의도적으로 넣은 비교 입력은 차이 1로 탐지 PASS. 기존 선택 이미지도 원본과 차이 0으로 손상 의심 배제
